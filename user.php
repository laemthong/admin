        ...
        if (move_uploaded_file($_FILES['user_photo']['tmp_name'], $upload_file)) {
            ...
            try {
                if (!empty($edit_user_id)) {
                    ...
                    // Update member_in_activity and activity if user_id is changed
                    if ($edit_user_id !== $user_id) {
                        $sql = "UPDATE member_in_activity SET user_id='$user_id' WHERE user_id='$edit_user_id'";
                        $conn->query($sql);

                        $sql = "UPDATE activity SET user_id='$user_id' WHERE user_id='$edit_user_id'";
                        $conn->query($sql);
                    ...
                    $sql = "INSERT INTO user_information (user_id, user_email, user_pass, user_name, user_age, user_photo, status)
                            VALUES ('$user_id', '$user_email', '$user_pass', '$user_name', '$user_age', '$upload_file', 'active')";
                    $conn->query($sql);
                }
